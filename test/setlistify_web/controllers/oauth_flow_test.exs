defmodule SetlistifyWeb.OAuthFlowTest do
  ...
  describe "Full OAuth authentication flow" do
    test "sign in, callback, and sign out flow works correctly", %{conn: conn, test_user: test_user} do
      ...
      callback_response = get(callback_conn, ~p"/oauth/callbacks/spotify?code=test_code&state=#{oauth_state}")
    ...
    test "sign in with invalid state shows error", %{conn: conn} do
      ...
      assert Phoenix.Flash.get(response.assigns.flash, :error) =~ "Response from Spotify did not match"
    ...
    test "sign in with failed token exchange shows error", %{conn: conn} do
      ...
      # Should show error message
      assert Phoenix.Flash.get(response.assigns.flash, :error) =~ "Failed to authenticate with Spotify"
